...
function generaEventos(){
    ...
    //Botones para borrar entrada y borrar todo
    document.getElementById("borrar_entrada").addEventListener("click",borrar_entrada);
}
...
function digitarNumero(){
    ...
    if(paso == 1){
        ...
        if(contarDecimales(num1)<7){
            console.log(contarDecimales(num1));
            num1+=""+valor;
            document.getElementById("lcd-value").innerText = num1;
        }
...
function calcular(){
    if(paso == 2){
        ...
        if (resultado % 1 !== 0) { // Verifica si el número tiene decimales
            document.getElementById("lcd-value").textContent = resultado.toFixed(7); // Redondea a 7 decimales
        } else {
            document.getElementById("lcd-value").textContent = resultado; // Muestra el valor tal cual si es un número entero
        }
        paso = 3;
    }
}
function contarDecimales(number){
    const cadena = number.toString();
    if(cadena.includes('.')){
